...
export default function Account() {
  ...
  const dummyEmails = [
    "Adam@brunel.ac.uk",
    "Marius@brunel.ac.uk",
    "Harshil@brunel.ac.uk",
    "Alex@brunel.ac.uk",
    "Aayisha@brunel.ac.uk",
  ];
  const dummyPhones = [
    "07115201776",
    "07289994594",
  ...
  const onSubmitAccountSettings = (e) => {
    ...
    const error = validateAccountSettings(accountSettings, {
      dummyUsernames,
      dummyEmails,
      dummyPhones,
      dummyPassword,
  ...
  const onSubmitSiteSettings = (e) => {
    ...
    if (
      (userSiteSettings.notification == "SMS Only" &&
        savedAccountSettings.phone_no == "") ||
      (userSiteSettings.notification === "Email and SMS" &&
        savedAccountSettings.phone_no == "")
    ) {
      alert(
        "You do not have an associated phone number. Enter a phone number, or select a different option."
      );
    ...
    alert(
      `Your site settings were successfully updated!\n\nTheme: ${theme}\nNotification Preferences: ${notification}\nTimeZone: ${timeZone}`
    );
  };
